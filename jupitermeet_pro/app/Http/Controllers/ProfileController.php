...
class ProfileController extends Controller
{
    ...
    {
        if (isDemoMode())
            return back()->with('error', __('This feature is not available in demo mode'));
        try {
    ...
    public function deleteAvatar(Request $request)
    {
        ...
        if (isDemoMode() && in_array($request->user()->email, $demoEmails))
    ...
    public function updateTfa(Request $request)
    {
        ...
        if (!getSetting('MAIL_USERNAME') && $request->tfa == 'active')
            return back()->with('error', __('SMTP settings are missing. Please try again later.'));
    ...
    public function uploadAvatar(Request $request)
    {
        ...
        if (isDemoMode() && in_array($request->user()->email, $demoEmails))
            return response()->json(['error' => __('This feature is not available in demo mode')]);
